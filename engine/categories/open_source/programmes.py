...
def parse_programme(seed: Dict, html: str, checked_at: Optional[datetime] = None, final_url: Optional[str] = None) -> Tuple[Optional[Dict], Dict]:
    ...
    record = _base(seed, checked, evidence)
...
def collect(fetch: Callable[[str], str] = _default_fetch, checked_at: Optional[datetime] = None, lake_path: str = OPPORTUNITIES_PATH, observations_path: str = OBSERVATIONS_PATH) -> Dict:
    records, observations = [], []
    for seed in SEEDS:
        try:
            fetched = fetch(seed["official_url"])
            html, final_url = fetched if isinstance(fetched, tuple) else (fetched, None)
            record, observation = parse_programme(seed, html, checked_at, final_url)
    ...
    merged = merge_programmes(records, observations, lake_path, observations_path)
    verification_now = datetime.now(timezone.utc).isoformat(timespec="seconds")
    merged = apply_verifications(merged, load_verifications(), verification_now)
    _atomic_json(lake_path, merged)
    return {"records": records, "observations": observations, "merged_count": len(merged)}


def _apply_verifications_cli() -> None:
    ...
    updated = apply_verifications(rows, verifications, datetime.now(timezone.utc).isoformat(timespec="seconds"))
