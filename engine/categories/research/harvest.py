"""Research-only external directory harvester.

This module is intentionally category-local: it reads a small research hub
 directory, discovers links, and writes only the requested output records. It
never writes the lake, S3, Supabase, operations data, or any shared collector
state. The category is sourced from each hub entry rather than hard-coded into
record construction so the harvester can be reused for another valid directory
of category-local hubs.

The CLI is a dry-run by design. Its default output is /tmp/research_dryrun.json;
repository, lake, and S3 destinations are rejected. It uses only Python's
standard library and does not call categories.research.research or any
programme_core collection/merge method.
"""
...
DEFAULT_OUTPUT = Path("/tmp/research_dryrun.json")
USER_AGENT = (
    "OpportunityRadarResearchHarvester/1.0 "
...
CANDIDATE_TERMS = re.compile(
    r"(?:research|reu|fellowship|summer[\s_-]*research|internship|"
    r"undergraduate[\s_-]*research)",
    re.IGNORECASE,
)
...
def excluded_domain(host: str) -> bool:
    ...
    return domain in EXCLUDED_REGISTERED_DOMAINS
...
def candidate_from_link(
    ...
    anchor: str,
    *,
    candidate_terms: Optional[re.Pattern[str]] = None,
    allow_same_origin: bool = False,
) -> Optional[str]:
    """Return a flagged candidate, with category-local origin policy.

    The default remains the Research policy: directory links must leave the
    hub's registered domain.  Authoritative category hubs may opt into
    same-origin programme/application links explicitly; callers still supply
    their own discovery terms rather than changing the global research terms.
    """
    if not raw_href:
        return None
    ...
    hub_parsed = urllib_parse.urlsplit(hub_url)
    if parsed.scheme not in VALID_SCHEMES:
        return None
    same_origin = host_key_for(parsed) == host_key_for(hub_parsed)
    same_registered_domain = registered_domain(parsed.hostname or "") == registered_domain(hub_parsed.hostname or "")
    if same_origin and not allow_same_origin:
        return None
    # A same-origin authoritative link is allowed; a sibling host on the hub's
    # registered domain is still not an official candidate boundary.
    if same_registered_domain and not same_origin:
        return None
    if excluded_domain(parsed.hostname or ""):
        return None
    terms = candidate_terms or CANDIDATE_TERMS
    if not (terms.search(anchor) or terms.search(normalized)):
        return None
    return normalized
...
def load_hubs(
    ...
    *,
    ...
    """Load and validate a category-local hub directory.

    Research accepts optional authoritative provenance for both its legacy
    records and the newer explicit schema, while retaining strict field,
    category, URL, and timestamp validation. Category wrappers can narrow the
    bounds and field policy explicitly (Open Source, for example, remains at
    its own 50-hub cap).
    """
    try:
    ...
    if not isinstance(value, list):
        raise ValueError("{} must contain a JSON list of hub objects".format(path))
    if exact_count is not None and len(value) != exact_count:
        raise ValueError("{} must contain exactly {} hub objects".format(path, exact_count))
    if len(value) < min_hubs or len(value) > max_hubs:
        raise ValueError("{} must contain between {} and {} hub objects".format(path, min_hubs, max_hubs))
    required = {"hub_id", "url", "category", "type", "added_at"}
    allowed = required | ({"authoritative"} if allow_authoritative else set())
    ...
    for index, item in enumerate(value, 1):
        if not isinstance(item, dict) or not set(item).issubset(allowed) or not required.issubset(item):
            ...
            raise ValueError("hub {} must have only these fields and all required fields: {}".format(index, fields))
        if not all(isinstance(item[key], str) and item[key].strip() for key in required):
            raise ValueError("hub {} required fields must be non-empty strings".format(index))
        if allow_authoritative and "authoritative" in item and not isinstance(item["authoritative"], bool):
            raise ValueError("hub {} authoritative must be boolean".format(index))
        if item["category"] != category or item["type"] != "hub":
            raise ValueError("hub {} must have category={} and type=hub".format(index, category))
        ...
        if allow_authoritative:
            normalized_item.setdefault("authoritative", False)
        seen_ids.add(item["hub_id"])
        seen_urls.add(normalized)
        hubs.append(normalized_item)
    return hubs
...
def fetch_hub(fetcher: Fetcher, hub: Dict[str, str]) -> Tuple[str, List[Tuple[str, str]], str]:
    """Return (state, links, reason), keeping a failed hub distinct from empty."""
    hub_url = hub["url"]
    if is_github_repo(hub_url):
        ...
        api_reason = api_result.reason or api_result.state

        repo_metadata: Optional[Dict[str, object]] = None
        repo_reason = ""
        needs_repo = markdown is not None and not (
            api_metadata and isinstance(api_metadata.get("html_url"), str)
        )
        if markdown is None or needs_repo:
            repo_result = fetcher.fetch(
                github_repo_api_url(hub_url),
                {"Accept": "application/vnd.github+json"},
            )
            if repo_result.state == "live" and repo_result.status == 200:
                try:
                    value = json.loads(repo_result.body)
                    if isinstance(value, dict):
                        repo_metadata = value
                    else:
                        repo_reason = "github repository metadata invalid"
                except json.JSONDecodeError:
                    repo_reason = "github repository metadata invalid JSON"
            else:
                repo_reason = repo_result.reason or repo_result.state

        if markdown is not None:
            branch = "HEAD"
            if repo_metadata and isinstance(repo_metadata.get("default_branch"), str):
        ...
        raw_url = github_raw_url(owner_repo[0], owner_repo[1], branch, path) if owner_repo and branch else None
        raw_result: Optional[FetchResult] = None
        if raw_url:
        ...
        if fallback.state == "live" and fallback.status == 200:
            try:
                return "processed", parse_html_links(fallback.body), "github_html_fallback; {}".format(fallback.reason or "robots_allow")
            except (TypeError, ValueError) as exc:
                return "failed", [], "github HTML parse failed: {}".format(exc)
        reasons = [api_reason]
        if repo_reason:
            reasons.append("repository metadata: " + repo_reason)
        if raw_result is not None:
            reasons.append("raw fallback: " + (raw_result.reason or raw_result.state))
        reasons.append("HTML fallback: " + (fallback.reason or fallback.state))
        reason = "; ".join(reasons)
        if any(result.state == "blocked" for result in (api_result, raw_result, fallback) if result is not None):
...
def discover_candidates(
    ...
    *,
    candidate_terms: Optional[re.Pattern[str]] = None,
    authoritative_same_origin: bool = False,
) -> Tuple[List[Candidate], Dict[str, str], Dict[str, str], Dict[str, Optional[int]], int, int]:
    """Discover deduplicated candidates using explicit category-local policy."""
    ...
    for hub in hubs:
        ...
        state, links, reason = fetch_hub(fetcher, hub)  # type: ignore[arg-type]
        ...
        if state == "failed":
            ...
            blocks += 1
        local: Set[str] = set()
        allow_same_origin = authoritative_same_origin and bool(hub.get("authoritative", False))
        for raw_href, anchor in links:
            candidate_url = candidate_from_link(
                str(hub["url"]), raw_href, anchor,
            ...
            candidate.memberships.setdefault(hub_id, (str(hub["url"]), clean_text(anchor)))
        counts[hub_id] = len(local) if state == "processed" else None
    return list(candidates.values()), hub_states, hub_reasons, counts, failures, blocks
...
def build_corroborating_hubs(
    candidate: Candidate, hubs: Sequence[Dict[str, object]]
) -> List[Dict[str, object]]:
    ...
    for membership_id in ordered_memberships:
        ...
        if configured is not None and "authoritative" in configured:
            evidence["authoritative"] = bool(configured.get("authoritative"))
...
def build_records(candidates: Sequence[Candidate], hubs: Sequence[Dict[str, str]], checked_at: str) -> List[Dict[str, object]]:
    ...
    for candidate in sorted(candidates, key=lambda item: item.url):
        ...
        source_hub, anchor = candidate.memberships[winner_id]
        name = anchor or fallback_name(candidate.url)
        ...
        records.append(
            {
                ...
                "programme_id": "research-programme-" + digest,
...
def guarded_output_path(value: str) -> Path:
    ...
    try:
        ...
        raise ValueError("output path must not be inside the repository")
    if any(part.lower() == "lake" for part in resolved.parts):
        raise ValueError("output path must not contain a lake path component")
...
def print_stats(
    ...
    for hub in hubs:
        ...
        print("hub {}: state={}, candidates={}, reason={}".format(
            hub_id, hub_states[hub_id], count_text, reason
        ))
    ...
    print("total unique opportunities: {}".format(len(records)))
...
def run(output: Path) -> int:
    ...
    fetcher = Fetcher()
    try:
        candidates, states, reasons, counts, failures, blocks = discover_candidates(hubs, fetcher)
    ...
    print_stats(hubs, states, reasons, counts, failures, blocks, candidates, records, live, fetcher)
    print("output: {}".format(output))
    return 0


def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Dry-run research hub directory harvester")
    parser.add_argument("--output", default=str(DEFAULT_OUTPUT), help="JSON output path (default: /tmp/research_dryrun.json)")
